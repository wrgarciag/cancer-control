...
Nx_Dx_mx_fx_1y_ref_year <- full_join(
      x  = country_acmx[, !"cause"],
      y  = country_frt %>% filter(year == reference_year),
      by = join_by(age, year, sex, location)
) %>%
      # mutate(year = reference_year) %>% #----------------------------Q: double check this#
      select(age, year, sex, location, Nx, Dx = acDx, mx = acmx, fx = asfr) %>%
...
for (i in seq_along(strt_yr:end_yr) + 1) {
      ...
      bsln_current_output <- proj_ccpm_markov(pop_data     = bsln_current_output,
                                              markov_input = bsln_markov_trace,
                                              fx_input     = fixed_fx,
                                              target_input = target_cancers,
                                              omx_input    = fixed_omx) %>%
            mutate(iteration = i,
                   scen      = "bsln",
                   Dx        = round(mx * Nx))
